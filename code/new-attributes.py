...
def train_attribute(attribute_id, C, split=0):
    ...
    if split == 0:
        train_classes=loadstr('../trainclasses.txt')
        test_classes=loadstr('../testclasses.txt')
    else:
        classnames = loadstr('../classnames.txt')
        startid= (split-1)*10
        stopid = split*10
    ...
    labels_trn = Features.BinaryLabels(Lplatt_trn)
